#!/bin/bash
# Build script for Linux/ARM64 using Docker or Podman
# Uses Docker/Podman with cross-compilation or QEMU emulation
...
echo -e "${YELLOW}Building for Linux/ARM64 using ${CONTAINER_RUNTIME}${NC}"
echo ""
...
# Docker image name
IMAGE_NAME="rust-bottle-linux-arm64"
DOCKERFILE="$SCRIPT_DIR/Dockerfile.linux-arm64"
...
for features in "${FEATURES[@]}"; do
    ...
    if [ -z "$features" ]; then
        CARGO_BUILD="cargo build --target aarch64-unknown-linux-gnu --release"
        CARGO_TEST="cargo test --target aarch64-unknown-linux-gnu"
    else
        CARGO_BUILD="cargo build --target aarch64-unknown-linux-gnu --features $features --release"
        CARGO_TEST="cargo test --target aarch64-unknown-linux-gnu --features $features"
    fi
    
    # Build
    # Note: Podman may not support --platform flag in all versions, but it's harmless
